...
// 当前写死了， 只执行一次的策略。
func MultiStepPortfolio(broker model.Broker, req *model.ContractPortfolioReq) {
	...
	case model.StrategyOutVolatility:
		{
			if !position.Count.Equal(decimal.Zero) {
				if position.Count.GreaterThan(decimal.Zero) {
					err = broker.AddOrder(req.Contract, model.OrderTypeSell, position.Count.Abs(), req.StrategyResult.Price, req.StrategyResult.Reason, req.Ts)
				} else {
					err = broker.AddOrder(req.Contract, model.OrderTypeBuy, position.Count.Abs(), req.StrategyResult.Price, req.StrategyResult.Reason, req.Ts)
				}
			}
		}
	case model.StrategyOutLong:
		{
			if position.IsEmpty() {
				err = broker.AddOrder(req.Contract, model.OrderTypeBuy, count, req.StrategyResult.Price, req.StrategyResult.Reason, req.Ts)
	...
	case model.StrategyOutShort:
		{
			if position.IsEmpty() {
				err = broker.AddOrder(req.Contract, model.OrderTypeSell, count, req.StrategyResult.Price, req.StrategyResult.Reason, req.Ts)
			} else if position.Count.GreaterThan(decimal.Zero) {
				err = broker.AddOrder(req.Contract, model.OrderTypeSell, position.Count.Abs(), req.StrategyResult.Price, req.StrategyResult.Reason, req.Ts)
				err = broker.AddOrder(req.Contract, model.OrderTypeSell, count, req.StrategyResult.Price, req.StrategyResult.Reason, req.Ts)
			} else if position.Count.LessThan(decimal.Zero) {
