...
func Kama(inReal []float64, erPeriod int, shortPeriod, longPeriod int) []float64 {

	outReal := make([]float64, len(inReal))

	constMax := float64(shortPeriod) / (float64(longPeriod) + 1.0)
	constDiff := float64(shortPeriod)/(float64(shortPeriod)+1.0) - constMax
	lookbackTotal := erPeriod
	startIdx := lookbackTotal
	sumROC1 := 0.0
	today := startIdx - lookbackTotal
	trailingIdx := today
...
func Ma(inReal []float64, inTimePeriod int, inMAType MaType) []float64 {
	...
	case KAMA:
		outReal = Kama(inReal, inTimePeriod, 2, 30)
	case MAMA:
