...
type Crawler interface {
	CrawlContractDaily(ctx context.Context, market *model.TradeObject, startDate, endDate time.Time) ([]*model.KLineNode, error)
	CrawlContractMinute(ctx context.Context, market *model.TradeObject, startDate, endDate time.Time) ([]*model.KLineNode, error)
	CrawlContractWeekly(ctx context.Context, market *model.TradeObject, startDate, endDate time.Time) ([]*model.KLineNode, error)
}
...
func CrawlContinueMainContractBySubject(ctx context.Context, subjectName string, handler Crawler) {
	subject, err := mongo.QuerySubject(ctx, subjectName)
	if err != nil {
		logs.Info("query all subject error ", err)
		return
	...
	contractDo := &model.ContractDO{
		ContractCnName: subject.CNName,
		ContractDate:   model.MainContinuous,
	}
...
func CrawlContractByContract(ctx context.Context, contract *model.TradeObject, handler Crawler) error {
	logs.Info("start to crawl crawlAllMainContractBySubjectName %v", contract.CNName)
	if contract == nil {
		panic("should not reach here")
	}
	if contract.ContractInfo.OfflineTime != "" {
