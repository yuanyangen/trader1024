package indicator

//import (
//	"fmt"
//	"github.com/go-echarts/go-echarts/charts"
//	"github.com/yuanyangen/trader1024/engine/indicator_base"
//	"github.com/yuanyangen/trader1024/engine/model"
//	"github.com/yuanyangen/trader1024/engine/talib"
//	"github.com/yuanyangen/trader1024/engine/utils"
//	"math"
//)
//
//type KAMAIndicator struct {
//	*indicator_base.IndicatorCommon
//	parentLine  *model.BaseLine
//	KAMALine    *indicator_base.Line
//	erPeriod    int64
//	shortPeriod int64
//	longPeriod  int64
//}
//
//func NewKAMAIndicator(kline *model.BaseLine, period, shortPeriod, longPeriod int64) *KAMAIndicator {
//	kama := &KAMAIndicator{
//		IndicatorCommon: indicator_base.NewIndicatorCommon(),
//		erPeriod:        period,
//		shortPeriod:     shortPeriod,
//		longPeriod:      longPeriod,
//		KAMALine:        indicator_base.NewLine(model.LineType_Day, fmt.Sprintf(kline.Name()+"_%v", period)),
//		parentLine:      kline,
//	}
//	kline.AddChildrenIndicator(kama)
//	return kama
//}
//
//func (kama *KAMAIndicator) Name() string {
//	return fmt.Sprintf("%v_KAMA_%v", kama.parentLine.Name(), kama.erPeriod)
//}
//
//func (kama *KAMAIndicator) AddData(ts int64, node model.DataNode) {
//	data_crawler, err := kama.parentLine.GetLastByTsAndCount(ts, kama.erPeriod+1)
//	if err != nil {
//		kama.KAMALine.AddData(ts, 0)
//		return
//	}
//	in := make([]float64, len(data_crawler))
//	for i, knode := range data_crawler {
//		in[i] = knode.GetValue()
//	}
//	out := talib.CustomKama(in, int(kama.erPeriod), int(kama.shortPeriod), int(kama.longPeriod), kama.GetCurrentFloat(data_crawler[len(data_crawler)-2].GetTs()))
//	avg := out[len(out)-1]
//	kama.KAMALine.AddData(ts, avg)
//}
//func (kama *KAMAIndicator) GetAllSortedData() []model.DataNode {
//	return nil
//}
//
//func (kama *KAMAIndicator) GetCurrentValue(ts int64) model.DataNode {
//	if kama.KAMALine == nil {
//		panic("KAMALine error")
//	}
//	if kama.erPeriod == 0 {
//		panic("erPeriod empty")
//	}
//	data_crawler, _ := kama.KAMALine.GetByTs(ts)
//	return data_crawler
//}
//
//func (kama *KAMAIndicator) GetCurrentFloat(ts int64) float64 {
//	v := kama.GetCurrentValue(ts)
//	if v != nil {
//		return v.GetValue()
//	}
//	return 0
//}
//func (kama *KAMAIndicator) GetLastByTsAndCount(ts int64, period int64) ([]model.DataNode, error) {
//	if kama.KAMALine == nil {
//		panic("KAMALine error")
//	}
//	if kama.erPeriod == 0 {
//		panic("erPeriod empty")
//	}
//	dataI, err := kama.KAMALine.GetLastByTsAndCount(ts, period)
//	if err != nil {
//		return nil, err
//	}
//	res := make([]model.DataNode, len(dataI))
//	for i, v := range dataI {
//		res[i] = v
//	}
//
//	return res, nil
//}
//func (kama *KAMAIndicator) GetByTs(ts int64) (model.DataNode, error) {
//	if kama.KAMALine == nil {
//		panic("KAMALine error")
//	}
//	if kama.erPeriod == 0 {
//		panic("erPeriod empty")
//	}
//	return kama.KAMALine.GetByTs(ts)
//}
//func (kama *KAMAIndicator) DoPlot(kline *charts.Line, ratioLine *charts.Line) {
//	allData := kama.KAMALine.GetAllSortedData()
//	x := make([]string, len(allData))
//	y := make([]float64, len(allData))
//	for i, v := range allData {
//		x[i] = utils.TsToDateString(v.GetTs())
//		y[i] = v.GetValue()
//	}
//	line := charts.NewLine()
//	line.SetGlobalOptions(charts.TitleOpts{Title: kama.Name()})
//	line.AddXAxis(x).AddYAxis(kama.Name(), y, charts.LineOpts{ConnectNulls: false})
//	kline.Overlap(line)
//}
//
//func (kama *KAMAIndicator) kama(values []float64, period, fastEma, slowEma int) []float64 {
//	if len(values) <= period {
//		panic("data_crawler error")
//	}
//	fastAlpha := 2.0 / (float64(fastEma) + 1.0)
//	slowAlpha := 2 / (float64(slowEma) + 1.0)
//	kamaV := float64(0.0)
//	result := []float64{}
//	change := []float64{}
//	for i := range values {
//		if i == 0 {
//			continue
//		}
//		change = append(change, math.Abs(values[i]-values[i-1]))
//		if i-period < 0 {
//			continue
//		}
//		mon := math.Abs(values[i] - values[i-period])
//		vol := 0.0
//		for j := len(change) - period - 1; j < len(change); j++ {
//			vol += change[j]
//		}
//		er := 0.0
//		if vol != 0 {
//			er = mon / vol
//		}
//		alpha := math.Pow(er*(fastAlpha-slowAlpha)+slowAlpha, 2)
//		kamaV = alpha*values[i] + (1-alpha)*kamaV
//
//		result[i] = kamaV
//	}
//	return result
//}
